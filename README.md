...
- Подключение локального репозитория к удалённому
  ...
  - Подключение
  - Отпарвка файлов на удаленный репозиторий
---
...
Замечание: для отправки на удалённый репозиторий была созданна отдельная ветка - global
...
После того как мы подключились к нашему удалённому репозиторию, можно приступить  к отправки файлов

``` 
git add README.md screenshots # добавляем в staging наши файлы

git commit -m "Добавлен README и screenshots" # делаем коммит

git push origin global # отправляем файлы (пушим)
```
![Изображение](screenshots/global_2.png)
